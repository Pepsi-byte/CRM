...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#uname").keydown(function(event){
			...
			if(event.keyCode==13){
				//阿松一个ajax请求
				$.ajax({
					url: "workbench/clue/getActivityWithClueNoHadClue.do",
					data: {
						"activityName": $.trim($("#uname").val()),
						"clueId": "${c.id}"
					},
					type: "get",
					dataType: "json",
			...
			//列表展现完成之后，模态窗口会自动的进行刷新，要记得将他关闭掉
			return false;
			}
		})
		...
		$("#addRelationBtn").click(function () {
			...
			if($xz.length==0){
				alert("请选择需要关联的市场活动按钮！");

				//长度大于0，也就是肯定是勾选了按钮的，可能是一个，也可能是多个
				...
				$.ajax({
					url: "workbench/clue/setRelation.do",
					data: param,
					type: "post",
					dataType: "json",
					success:function(data){
						if(data.success){
							//关联成功，刷新数据列表
							showActivityList();
							//清楚搜索框中的信息，清楚复选框按钮，清空activitySearchBody中的内容
	...
	function showActivityList() {
		//页面加载完之后，发一个ajax请求，更是线索详情中市场活动的信息
		$.ajax({
	...
	//一点击接触关系按钮，调用这个方法
	function unbund(id) {
		//发送一个ajax请求，根据id删除线索与市场活动关联的信息表中的数据，就是解除市场活动与线索的关联关系
		$.ajax({
					...
					<div class="btn-group" style="position: relative; top: 18%; left: 8px;">
						<form class="form-inline" role="form">
						  <div class="form-group has-feedback">
							  <input type="text" class="form-control" id="uname" style="width: 300px;" placeholder="请输入市场活动名称，支持模糊查询123">
...
<%--							</tr>--%>
	...
	<div style="position: relative; top: -70px;">
		...
		<div style="position: relative; left: 40px; height: 30px; top: 50px;">
			...
			<div style="width: 500px;position: relative; left: 200px; top: -20px;"><b>${c.createBy}&nbsp&nbsp;&nbsp;</b><small style="font-size: 10px; color: gray;">${c.createTime}</small></div>
